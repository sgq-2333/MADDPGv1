...
from Env_Config_target.core import Agent
    ...
    def draw(self):
        ...
        for e, entity in enumerate(self.world.entities):
            ...
            if 'border' in entity.name:
                ...
                # pygame.draw.rect(self.screen, (0, 0, 0), rect, 1)  # borders
                ...
                if isinstance(entity, Agent):
                    if np.linalg.norm(entity.state.p_vel) > 0:  # 只有当速度向量不为零时才绘制
                        vel_direction = entity.state.p_vel / np.linalg.norm(entity.state.p_vel)  # 归一化速度向量
                        # vel_direction = entity.state.p_vel
                        ...
                        # 计算箭头两个斜边的端点
                        arrow_length = 10
                        arrow_angle = np.pi / 6  # 箭头的夹角，可以根据需要调整
                    ...
                    #     vel_direction = entity.state.p_vel / np.linalg.norm(entity.state.p_vel)  # 归一化速度向量
                    #     vel_pos = (x, y)
                    #     vel_end = (int(x + vel_direction[0] * 20), int(y + vel_direction[1] * 20))
                    #     pygame.draw.aaline(self.screen, (0, 0, 0, 128), vel_pos, vel_end, 2)  # 透明红色线表示速度向量的两个斜边

                    #三角形
                    # vel_x, vel_y = entity.state.p_vel
                    # vel_direction = np.array([vel_x, vel_y])
                    # if np.linalg.norm(vel_direction) != 0:
                    #     vel_direction /= np.linalg.norm(vel_direction)  # 将速度方向归一化
                    #
                    # triangle_points = [
                    #     (x, y),
                    #     (x + 20 * vel_direction[0], y - 20 * vel_direction[1]),  # 根据速度方向确定三角形的尖端位置
                    #     (x + 20 * (-vel_direction[1]), y + 20 * vel_direction[0])  # 根据速度方向确定三角形的一个侧边顶点位置
                    # ]
                    # pygame.draw.polygon(self.screen, (255, 0, 0), triangle_points)  # 绘制探测三角形
            #####################################
            # assert (
