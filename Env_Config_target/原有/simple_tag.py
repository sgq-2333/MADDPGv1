...
class Scenario(BaseScenario):
    ...
    def get_collision_risk_level(self, dist, safety_radius):
        """计算碰撞风险等级"""
        if dist >= safety_radius:
            return 0.0
        return (safety_radius - dist) / safety_radius
    ...
    def agent_reward(self, agent, world):
        ...
        if not agent.adversary:  # 入侵者奖励
            for target in world.selected_target:
